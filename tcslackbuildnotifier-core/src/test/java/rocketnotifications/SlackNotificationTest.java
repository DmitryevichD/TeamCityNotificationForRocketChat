...
/*
	...
	public void test_200() throws FileNotFoundException, IOException, Exception {
		...
		RocketNotification w = factory.getSlackNotification(httpClient, "#test-channel");
		w.setEnabled(true);
		w.post();
	...
	public void test_NotEnabled() throws FileNotFoundException, IOException, InterruptedException {
		...
		RocketNotification w = factory.getSlackNotification(url + "/200", proxy, proxyPort);
		w.post();
		stopWebServer(s);
	...
	public void test_200WithFilename() throws FileNotFoundException, IOException, Exception {
		SlackNotificationTestServer s = startWebServer();
		RocketNotification w = factory.getSlackNotification(url + "/200");
	...
	public void test_302() throws FileNotFoundException, IOException, Exception {
		SlackNotificationTestServer s = startWebServer();
		RocketNotification w = factory.getSlackNotification(url + "/302");
		w.setEnabled(true);
		w.post();
		stopWebServer(s);
	...
	public void test_302WithProxy() throws FileNotFoundException, IOException, InterruptedException {
		SlackNotificationTestServer s = startWebServer();
		SlackNotificationTestProxyServer p = startProxyServer();
		RocketNotification w = factory.getSlackNotification(url + "/302", proxy, proxyPort);
		w.setEnabled(true);
		w.post();
		stopWebServer(s);
		stopProxyServer(p);
	...
	public void test_404WithProxyStringPort() throws FileNotFoundException, IOException, InterruptedException {
		SlackNotificationTestServer s = startWebServer();
		SlackNotificationTestProxyServer p = startProxyServer();
		RocketNotification w = factory.getSlackNotification(url + "/404", proxy, proxyPortString);
		w.setEnabled(true);
		w.post();
		stopWebServer(s);		
		stopProxyServer(p);
	...
	public void test_404WithProxyConfig() throws FileNotFoundException, IOException, InterruptedException {
		...
		RocketNotification w = factory.getSlackNotification(url + "/404", pc);
		w.setEnabled(true);
		w.post();
		stopWebServer(s);
		stopProxyServer(p);
	...
	public void test_SlackNotificationCollection() throws SlackNotificationParameterReferenceException {
		...
		params.put("system.rocketnotifications.1.url", url);
		params.put("system.rocketnotifications.1.enabled", "true");
		params.put("system.rocketnotifications.1.parameter.1.name","fod");
		params.put("system.rocketnotifications.1.parameter.1.value","baa");
		params.put("system.rocketnotifications.1.parameter.2.name","slash");
		params.put("system.rocketnotifications.1.parameter.2.value","dot");
		params.put("system.rocketnotifications.2.url", url + "/something");
		params.put("system.rocketnotifications.2.enabled", "false");
		params.put("system.rocketnotifications.2.parameter.1.name","foo");
		params.put("system.rocketnotifications.2.parameter.1.value","bar");
	...
	public void test_SlackNotificationCollectionWithRecursiveParameterReference() throws SlackNotificationParameterReferenceException {
		...
		params.put("system.rocketnotifications.1.url", url);
		params.put("system.rocketnotifications.1.enabled", "true");
		params.put("system.rocketnotifications.1.parameter.1.name","foo");
		params.put("system.rocketnotifications.1.parameter.1.value","bar");
		params.put("system.rocketnotifications.1.parameter.2.name","slash");
		params.put("system.rocketnotifications.1.parameter.2.value","%system.test.recursive1%");
		SlackNotificationCollection whc = new SlackNotificationCollection(params);
		System.out.println("Test 1" + whc.getSlackNotifications().get(1).getParameterisedUrl());
		assertTrue(whc.getSlackNotifications().get(1).getChannel().equals(url));
    ...
    public void test_SlackNotificationCollectionWithPost() throws SlackNotificationParameterReferenceException, InterruptedException {
		Map <String, String> params = new HashMap<String, String>();
		//params.put("system.rocketnotifications.1.url", url + "/200");
		params.put("system.rocketnotifications.1.url", "http://localhost/slacknotifications/" );
		params.put("system.rocketnotifications.1.enabled", "true");
		params.put("system.rocketnotifications.1.parameter.1.name","fod");
		params.put("system.rocketnotifications.1.parameter.1.value","baa");
		params.put("system.rocketnotifications.1.parameter.2.name","slash");
		params.put("system.rocketnotifications.1.parameter.2.value","dot");
		params.put("system.rocketnotifications.2.url", "http://localhost/slacknotifications/test/" );
		params.put("system.rocketnotifications.2.enabled", "true");
		SlackNotificationCollection whc = new SlackNotificationCollection(params);
		SlackNotificationTestServer s = startWebServer();
		for (Iterator<RocketNotification> i = whc.getSlackNotificationsAsCollection().iterator(); i.hasNext();){
			RocketNotification wh = i.next();
			try {
					wh.post();					
