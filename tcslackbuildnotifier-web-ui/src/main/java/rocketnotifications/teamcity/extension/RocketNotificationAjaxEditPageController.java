...
import rocketnotifications.teamcity.settings.SlackNotificationContentConfig;
...
public class RocketNotificationAjaxEditPageController extends BaseController {

	    private static final String PROJECT_ID = "projectId";
	    private static final String SLACK_NOTIFICATION = "rocketNotifications";
	...
	public RocketNotificationAjaxEditPageController(SBuildServer server, WebControllerManager webManager,
                                                    ProjectSettingsManager settings, RocketNotificationProjectSettings whSettings, RocketNotificationPayloadManager manager,
                                                    PluginDescriptor pluginDescriptor, RocketNotificationMainSettings mainSettings) {
	        super(server);
	    ...
	    public void register(){
	      myWebManager.registerController("/rocketnotifications/ajaxEdit.html", this);
	    }
	    ...
	    protected ModelAndView doHandle(HttpServletRequest request, HttpServletResponse response) throws Exception {
	        ...
	        SProject myProject;
	        RocketNotificationProjectSettings projSettings = null;

	    	if ("post".equalsIgnoreCase(request.getMethod())){
	    		if ((request.getParameter(PROJECT_ID) != null)
	    			&& request.getParameter(PROJECT_ID).startsWith("project")){
	    		    	projSettings = (RocketNotificationProjectSettings) mySettings.getSettings(request.getParameter(PROJECT_ID), ROCKET_NOTIFICATIONS_SETTINGS_ATTRIBUTE_NAME);
			    			...
			    			if ((request.getParameter(SUBMIT_ACTION) != null ) 
			    				...
			    				&& (request.getParameter("removedSlackNotificationId") != null)){
			    					...
			    					if(projSettings.updateSuccessful()){
			    						...
			    						params.put(MESSAGES, "<errors><error id=\"messageArea\">The rocketnotifications was not found. Have the SlackNotifications been edited on disk or by another user?</error></errors>");
			    					}
	    	...
	    	if ("get".equalsIgnoreCase(request.getMethod())
		    	...
		    	RocketNotificationProjectSettings projSettings1 = (RocketNotificationProjectSettings) mySettings.getSettings(request.getParameter(PROJECT_ID), ROCKET_NOTIFICATIONS_SETTINGS_ATTRIBUTE_NAME);
	        ...
	        return new ModelAndView(myPluginPath + "RocketNotification/ajaxEdit.jsp", params);
	    }
