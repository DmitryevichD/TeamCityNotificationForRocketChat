 ...
 * Listens for Server events and then triggers the execution of rocketnotifications if configured.
 */
public class RocketNotificationListener extends BuildServerAdapter {

    public static final String ROCKET_NOTIFICATIONS_SETTINGS_ATTRIBUTE_NAME = "rocketNotifications";
    ...
    private RocketNotificationMainSettings myMainSettings;
    private RocketNotificationPayloadManager myManager;
    private SlackNotificationFactory slackNotificationFactory;
	private NotificationUtility notificationUtility = new NotificationUtility();

    public RocketNotificationListener(){
        myBuildServer = null;
        mySettings = null;
        myMainSettings = null;
        myManager = null;
    ...
    public RocketNotificationListener(SBuildServer sBuildServer,
                                      ...
                                      RocketNotificationPayloadManager manager,
	...
	private void processBuildEvent(SRunningBuild sRunningBuild, BuildStateEnum state) {
			...
			Loggers.SERVER.debug("About to process Rocket notifications for " + sRunningBuild.getProjectId() + " at buildState " + state.getShortName());
	...
	private List<SlackNotificationConfigWrapper> getListOfEnabledSlackNotifications(String projectId) {
		...
		for (SProject project : projects){
			RocketNotificationProjectSettings projSettings = (RocketNotificationProjectSettings) mySettings.getSettings(project.getProjectId(), ROCKET_NOTIFICATIONS_SETTINGS_ATTRIBUTE_NAME);
		    	...
		    	for (SlackNotificationConfig whc : projSettings.getSlackNotificationsConfigs()){
		    		if (whc.isEnabledForSubProjects() == false && !myProject.getProjectId().equals(project.getProjectId())){
		    			...
		    			if (Loggers.ACTIVITIES.isDebugEnabled()){
			    			Loggers.ACTIVITIES.debug(this.getClass().getSimpleName() + ":getListOfEnabledSlackNotifications() "
			    					+ ":: subprojects not enabled. myProject is: " + myProject.getProjectId() + ". rocketnotifications project is: " + project.getProjectId());
		    			}
		    			continue;
		    		}
		    		...
		    		if (whc.getEnabled()){
		    			...
		    			Loggers.ACTIVITIES.debug(this.getClass().getSimpleName()
		    					+ ":processBuildEvent() :: RocketNotification disabled. Will not process " + whc.getChannel());
		    		}
				}
	    	} else {
	...
	/**
	 * An inner class to wrap up the RocketNotification and its SlackNotificationConfig into one unit.
	 *
	 */
