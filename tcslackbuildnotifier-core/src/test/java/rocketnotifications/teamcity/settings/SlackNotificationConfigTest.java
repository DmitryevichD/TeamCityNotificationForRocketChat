...
public class SlackNotificationConfigTest {
	...
	public void setup() throws JDOMException, IOException{
		...
		slacknotificationDisabled    = ConfigLoaderUtil.getFirstSlackNotificationInConfig(new File("src/test/resources/project-settings-test-rocketnotifications-disabled.xml"));
...
//		assertTrue("One and only one rocketnotifications expected when loading test config from file : " + f.getName(), fileAsElement.getChild("slackNotifications").getChildren("rocketnotifications").size() == 1);
//		return new SlackNotificationConfig((Element) fileAsElement.getChild("slackNotifications").getChildren("rocketnotifications").get(0));
//	}
    ...
    public void loading_config_when_custom_content_section_is_present_sets_customContent(){
        assertNotSame(RocketNotificationMainConfig.DEFAULT_BOTNAME, slacknotificationCustomContent.getContent().getBotName());
        assertNotSame(RocketNotificationMainConfig.DEFAULT_ICONURL, slacknotificationCustomContent.getContent().getIconUrl());
